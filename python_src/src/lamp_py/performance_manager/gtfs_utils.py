...
def service_date_from_timestamp(timestamp: int) -> int:
    ...
    date_and_time = datetime.datetime.fromtimestamp(
        timestamp, tz=pytz.timezone("EST")
    )

    service_date_cutoff = datetime.time(hour=3)
    if date_and_time.time() < service_date_cutoff:
...
def add_missing_service_dates(
    ...
    """
    events_dataframe["service_date"] = events_dataframe.apply(
        lambda record: service_date_from_timestamp(record[timestamp_key])
        if pandas.isna(record["service_date"])
        else record["service_date"],
        axis=1,
    )
