...
def trips_for_headways_subquery(service_date: int) -> sa.sql.selectable.Subquery:
    """
    ...
    return (
        ...
        .select_from(rt_trips_sub)
        .where(
            rt_trips_sub.c.stop_count > 1, # drop trips with one stop count, probably not valid
        )
        .order_by(
            rt_trips_sub.c.pm_trip_id,
            rt_trips_sub.c.parent_station,
